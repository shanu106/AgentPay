...
import SavedPaymentModal from './SavedPaymentModal';
...
const getStoredSession = () => {
  ...
  const email = localStorage.getItem('agentpay_email') || cookieEmail;
  if (!email) return null;
  const name = localStorage.getItem('agentpay_name') || (email.startsWith('shahnawaz') ? 'Shahnawaz Nilger' : 'Nawaz Khan');
  return { email, name };
};

function AgentDashboard({ user, token, onLogout }) {
  const initialSession = getStoredSession();
  const fallbackName = user?.name || initialSession?.name || 'Customer';
  const fallbackEmail = user?.email || initialSession?.email || '';
  ...
  const [customerName, setCustomerName] = useState(fallbackName);
  const [customerEmail, setCustomerEmail] = useState(fallbackEmail);
  ...
  useEffect(() => {
    ...
    loadInitialData(session?.email || fallbackEmail || 'shahnawaznilger@gmail.com');
  }, []);
  ...
  const toggleListening = () => {
    if (!userProfile || !userProfile.email || userProfile.email === 'guest@example.com') {
      setIsAuthOpen(true);
      return;
    }
  ...
  const handlePaymentSuccess = (paymentResult) => {
    ...
    const successReply = `🎉 Order confirmed and paid successfully for ${updatedOrder?.productTitle || 'your purchase'} (${updatedOrder?.orderId || 'N/A'}).`;
    ...
    setSteps(prev => [
      ...
      { text: `Order Complete!`, status: 'completed' }
    ]);
    ...
    <div className="app-layout">
      ...
      <aside className={`sidebar ${sidebarOpen ? 'sidebar-open' : ''}`}>
        ...
        <div className="sidebar-menu">
          ...
          <div className="menu-group">
            <span className="menu-label">SUPPORT &amp; DOCS</span>
            <button
              type="button"
          ...
          <div className="top-actions">
            ...
            <div className="user-profile-badge" onClick={() => setIsAuthOpen(true)}>
              ...
              <span className="user-name-text">
                {userProfile?.name ? userProfile.name : 'Sign In / Profile'}
              </span>
            </div>
          </div>
              ...
              <div className="metric-grid-4">
                ...
                <div className="stat-card">
                  ...
                  <div className="stat-card-value">
                    ₹{(spendingStats?.remaining !== undefined ? spendingStats.remaining : Math.max(0, (spendingStats?.dailyLimit || policyData?.daily_spending_limit || 50000) - (spendingStats?.spentToday || 0))).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                  </div>
                  <div className="stat-card-footer">
                    <span>Limit: ₹{(spendingStats?.dailyLimit || policyData?.daily_spending_limit || 50000).toLocaleString()}</span>
                ...
                <div className="stat-card">
                  ...
                  <div className="stat-card-footer">
                    <span>Reset at {spendingStats?.resetTime || '00:00 (Midnight)'}</span>
              ...
              <div className="main-workspace-grid">
                  ...
                  <div className="ai-terminal-panel">
                    ...
                    {agentResult && (
                      <div style={{ marginTop: '18px', padding: '16px', borderRadius: '10px', background: 'var(--bg-input)', border: '1px solid var(--border-subtle)' }}>
                        ...
                        {(agentResult.requiresCheckout || agentResult.requiresConfirmation) && !confirmedOrder && (
                          <button
                            type="button"
                            className="btn-primary-blue"
                            style={{ width: '100%', marginTop: '12px', justifyContent: 'center', padding: '12px 18px', fontSize: '0.92rem', fontWeight: '700' }}
                            onClick={() => setIsRazorpayOpen(true)}
                          >
                            <span>💳 Confirm & Complete Razorpay Checkout (₹{(activeOrder?.amount || agentResult?.order?.amount || 0).toLocaleString()})</span>
                            <ArrowRight size={16} />
                          </button>
                        )}
                      </div>
                      ...
                      <tbody>
                        {recentOrders.length === 0 ? (
                          ...
                          recentOrders.slice(0, 5).map((ord) => {
                            const isPaid = ord.payment_status === 'paid' || ord.paymentStatus === 'paid' || ord.status === 'confirmed' || ord.status === 'order_confirmed' || ord.status === 'completed';
                            const isDenied = ord.status === 'denied' || ord.status === 'failed';
                            const orderTitle = ord.product_title || ord.productTitle || 'Autonomous Purchase';
                            const orderId = ord.order_id || ord.orderId || 'ORD-SYNC';
                            const rzpPaymentId = ord.razorpay_payment_id || ord.razorpayPaymentId || ord.razorpay_order_id || ord.razorpayOrderId;

                            return (
                              <tr key={orderId}>
                                <td>
                                  <div className="table-item-name">{orderTitle}</div>
                                  <div className="table-item-sub">{orderId}</div>
                                </td>
                                <td style={{ fontWeight: 700, color: 'var(--text-primary)' }}>
                                  ₹{parseFloat(ord.amount || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                                </td>
                                <td>{new Date(ord.created_at || ord.createdAt || Date.now()).toLocaleDateString()}</td>
                                <td>
                                ...
                                <td style={{ fontSize: '0.78rem', color: 'var(--text-muted)' }}>
                                  {rzpPaymentId ? `Razorpay: ${rzpPaymentId}` : 'Autonomous Agent Checkout'}
                                </td>
                              </tr>
                            );
                          })
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                  ...
                  <AgentActivityPanel
                    ...
                    onConfirmCheckout={() => setIsRazorpayOpen(true)}
                  />
          ...
          {activeNav === 'terminal' && (
            <div style={{ maxWidth: '1000px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              <div className="ai-terminal-panel">
                ...
                {/* Active Order Result Card in Terminal View */}
