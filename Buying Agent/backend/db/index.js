...
async function initDatabase() {
  ...
  try {
    ...
    // Supports token_ref and legacy/display card_number
    await client.query(`
    ...
    // Ensure both card_number and token_ref columns exist
    try {
      await client.query(`ALTER TABLE payment_methods ADD COLUMN IF NOT EXISTS card_number VARCHAR(255)`);
    ...
    // ====== SEED DATA HELPER ======
    const seedUserAccount = async (email, name, phone) => {
      ...
      if (userRes.rows.length === 0) {
        ...
        const insertedUser = await client.query(
          ...
          [name, email, defaultPasswordHash, phone, 50000.00]
        );
        uId = insertedUser.rows[0].id;
        console.log(`[PostgreSQL] Seeded user ${email} (ID: ${uId})`);
      } else {
      ...
      `, [`addr_home_${uId}`, uId, name, `addr_office_${uId}`]);

      // Seed Payment Methods
      const seedPaymentMethods = [
        { id: `pm_visa_1007_${uId}`, type: 'card', method: 'card', brand: 'Visa (Domestic)', last4: '1007', card_number: '4100280000001007', token_ref: 'rzp_test_visa_1007', expiry: '12/28', holder: name, label: 'Visa Debit (•••• 1007)', category: 'Cards', auto_debit_limit: 15000.00, is_default: true },
        { id: `pm_icici_4022_${uId}`, type: 'card', method: 'card', brand: 'Amazon Pay ICICI Card', last4: '4022', card_number: '4022000000004022', token_ref: 'rzp_test_icici_4022', expiry: '08/29', holder: name, label: 'Amazon Pay ICICI (•••• 4022)', category: 'Cards', auto_debit_limit: 25000.00, is_default: false },
        { id: `pm_hdfc_3003_${uId}`, type: 'card', method: 'card', brand: 'HDFC Millennia Card', last4: '3003', card_number: '3003000000003003', token_ref: 'rzp_test_hdfc_3003', expiry: '05/30', holder: name, label: 'HDFC Millennia (•••• 3003)', category: 'Cards', auto_debit_limit: 20000.00, is_default: false },
        { id: `pm_rupay_1005_${uId}`, type: 'card', method: 'card', brand: 'RuPay Domestic Debit', last4: '1005', card_number: '6527658900001005', token_ref: 'rzp_test_rupay_1005', expiry: '11/27', holder: name, label: 'RuPay Debit (•••• 1005)', category: 'Cards', auto_debit_limit: 10000.00, is_default: false },
        { id: `nb_bob_${uId}`, type: 'netbanking', method: 'netbanking', brand: 'Bank of Baroda', bank: 'BARB_R', bank_name: 'Bank of Baroda NetBanking', holder: name, label: 'Bank of Baroda NetBanking', category: 'NetBanking', auto_debit_limit: 50000.00, is_default: false },
        { id: `nb_sbi_${uId}`, type: 'netbanking', method: 'netbanking', brand: 'State Bank of India', bank: 'SBIN', bank_name: 'State Bank of India', holder: name, label: 'SBI NetBanking', category: 'NetBanking', auto_debit_limit: 50000.00, is_default: false },
        { id: `nb_hdfc_${uId}`, type: 'netbanking', method: 'netbanking', brand: 'HDFC Bank', bank: 'HDFC', bank_name: 'HDFC Bank', holder: name, label: 'HDFC Bank NetBanking', category: 'NetBanking', auto_debit_limit: 50000.00, is_default: false },
        { id: `nb_icici_${uId}`, type: 'netbanking', method: 'netbanking', brand: 'ICICI Bank', bank: 'ICIC', bank_name: 'ICICI Bank', holder: name, label: 'ICICI Bank NetBanking', category: 'NetBanking', auto_debit_limit: 50000.00, is_default: false },
        { id: `upi_gpay_${uId}`, type: 'upi', method: 'upi', brand: 'Google Pay', vpa: `${email.split('@')[0]}@okaxis`, holder: name, label: `Google Pay (${email.split('@')[0]}@okaxis)`, category: 'UPI', auto_debit_limit: 20000.00, is_default: false },
        { id: `upi_phonepe_${uId}`, type: 'upi', method: 'upi', brand: 'PhonePe', vpa: `${email.split('@')[0]}@ybl`, holder: name, label: `PhonePe (${email.split('@')[0]}@ybl)`, category: 'UPI', auto_debit_limit: 20000.00, is_default: false }
      ];
      ...
      // Seed Agent Authorization
      const authCheck = await client.query('SELECT id FROM agent_authorizations WHERE user_id = $1', [uId]);
    ...
    // Seed both default user accounts
    await seedUserAccount('shahnawaznilger@gmail.com', 'Shahnawaz Nilger', '+91 9876543210');
    await seedUserAccount('nawaz@gmail.com', 'Nawaz Khan', '+91 9876543210');
